...
func create_2d_audio_at_location(location, typeName : String):
	var type : int = SoundEffectSettings.SOUND_EFFECT_TYPE.get(typeName)
...
func create_audio(typeName : String):
	var type : int = SoundEffectSettings.SOUND_EFFECT_TYPE.get(typeName)
	if sound_effect_dict.has(type):
		var sound_effect_setting : SoundEffectSettings = sound_effect_dict[type]
		if sound_effect_setting.has_open_limit():
